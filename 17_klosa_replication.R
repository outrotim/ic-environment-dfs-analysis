...
#
# Reduced indicator set used here:
...
#   "Qualitative external replication in KLoSA (Korea) using the reduced
#    indicator set available in Harmonized KLoSA E.2; cognition was not
#    replicated owing to absence of cognitive measures in KLoSA E.2."
#
...
suppressPackageStartupMessages({
  library(tidyverse); library(haven); library(lavaan); library(survival)
})
...
step1_load <- function() {
  ...
  write.csv(
    ...
    file.path(RESULTS_DIR, "klosa_indicator_cov.csv"), row.names = FALSE
  )
  long
}
...
step2_composites <- function(long) {
  ...
  '
  cat("  Fitting single-group 2-factor CFA on KLoSA...\n")
  fit <- tryCatch(
  ...
  if (!is.null(fit)) {
    ...
    cat("  |loading| weights (fallback=1 if weak):\n"); print(round(w, 3))
  } else {
...
step4_cox <- function(surv) {
  ...
  cat(sprintf("\n  *** KLoSA IC x Env interaction HR = %.3f (95%% CI %.3f-%.3f), p = %.4f ***\n",
