#!/usr/bin/env Rscript
# 09_sensitivity.R — Sensitivity Analyses (20 pre-specified)
...
# --- S14: Cause-specific hazard for death (disability as competing event) ---
# CSH-Disability is already captured as S5 (disability-only outcome).
# Here we model death as the event, treating disability as censoring.
# This completes the competing risks decomposition of DFS.
cat("S14: Cause-specific hazard for death (disability censored)...\n")
...
surv_s14 <- surv_base %>%
  ...
  mutate(
    ...
    cs_status = case_when(
      cs_time == t_disab & cs_time < Inf ~ 1L,    # disability first
      cs_time == t_death & cs_time < Inf ~ 2L,     # death without prior disability
      TRUE ~ 0L                                      # censored
    ),
    cs_time = pmax(cs_time, 0.001),
    event_death_cs = as.integer(cs_status == 2)     # death as event, rest censored
  )
...
for (coh in sort(unique(surv_s14$cohort))) {
  ...
  if (nrow(d) < 100 || n_events < 10) {
    cat(sprintf("  %s: SKIPPED (n=%d, death events=%d)\n", coh, nrow(d), n_events))
    next
  }
  ...
  if (!is.null(fit)) {
    ...
    stage1_cr <- bind_rows(stage1_cr, res)
  }
}
...
if (nrow(int_sub) >= 2) {
  ma_cr <- tryCatch(rma(yi = log_hr, sei = se_log_hr, data = int_sub, method = "REML"),
                    error = function(e) NULL)
  if (!is.null(ma_cr)) {
    s14 <- tibble(analysis = "S14_csh_death", term = "IC×Env_CSH_death",
                  hr = exp(as.numeric(ma_cr$beta)), ci_lo = exp(ma_cr$ci.lb),
                  ci_hi = exp(ma_cr$ci.ub), p = ma_cr$pval, i2 = ma_cr$I2,
                  n_cohorts = nrow(int_sub),
                  note = sprintf("CSH-death, K=%d, N=%s", nrow(int_sub),
                                 format(sum(int_sub$n), big.mark = ",")))
    all_sens <- bind_rows(all_sens, s14)
    cat(sprintf("  Done: CSH-Death pooled HR=%.3f (%.3f–%.3f), p=%.4f, K=%d\n",
                exp(as.numeric(ma_cr$beta)), exp(ma_cr$ci.lb), exp(ma_cr$ci.ub),
                ma_cr$pval, nrow(int_sub)))
  }
} else {
  all_sens <- bind_rows(all_sens, tibble(
    analysis = "S14_csh_death", term = "IC×Env_CSH_death",
    hr = NA, ci_lo = NA, ci_hi = NA, p = NA, i2 = NA,
    n_cohorts = nrow(int_sub),
    note = "Insufficient cohorts for CSH-death pooling"))
  cat("  CSH-Death: insufficient cohorts for pooling\n")
}
