...
#          results/reri_stage1.csv       — cohort-specific M5 RERI estimates
#          results/reri_stage2_pooled.csv — pooled M5 RERI with provenance
...
SCRIPT_DIR <- tryCatch(
  ...
  error = function(e) {
    normalizePath(Sys.getenv("STUDY17_PROJECT_DIR", unset = getwd()), mustWork = FALSE)
  }
)
...
compute_m5_reri <- function(fit, cohort, n, n_events) {
  terms <- c(
    joint = "ic_env_6grpLow IC / Low Env",
    low_ic = "ic_env_6grpLow IC / High Env",
    low_env = "ic_env_6grpHigh IC / Low Env"
  )
  beta <- stats::coef(fit)
  missing_terms <- setdiff(terms, names(beta))
  if (length(missing_terms) > 0) {
    stop("M5 RERI terms missing: ", paste(missing_terms, collapse = ", "))
  }

  beta_terms <- beta[terms]
  names(beta_terms) <- names(terms)
  covariance <- stats::vcov(fit)[terms, terms, drop = FALSE]
  hr <- exp(beta_terms)
  reri <- unname(hr[["joint"]] - hr[["low_ic"]] - hr[["low_env"]] + 1)
  gradient <- c(hr[["joint"]], -hr[["low_ic"]], -hr[["low_env"]])
  se <- sqrt(drop(t(gradient) %*% covariance %*% gradient))
  z_value <- reri / se

  tibble(
    cohort = cohort,
    model_source = "M5_6group",
    reference_group = "High IC / High Env",
    joint_group = "Low IC / Low Env",
    single_low_ic_group = "Low IC / High Env",
    single_low_env_group = "High IC / Low Env",
    hr_joint = unname(hr[["joint"]]),
    hr_low_ic = unname(hr[["low_ic"]]),
    hr_low_env = unname(hr[["low_env"]]),
    reri = reri,
    se = se,
    ci_lower = reri - 1.96 * se,
    ci_upper = reri + 1.96 * se,
    z_value = z_value,
...
step6_diagnostics <- function(surv) {
  ...
  # Estimate RERI within each cohort from M5, retaining coefficient covariance,
  # then pool the cohort-specific additive interaction estimates by REML.
  cat("  6b: RERI (additive interaction) from cohort-specific M5 models\n")
  cat("  RERI = HR_11 - HR_10 - HR_01 + 1\n")
  ...
  for (coh in sort(unique(surv$cohort))) {
    dat <- surv %>%
      filter(cohort == coh) %>%
      filter(!is.na(ic_env_6grp) & !is.na(age) & !is.na(female) & !is.na(chronic_n))
    if (nrow(dat) < 50 || sum(dat$event_dfs) < 20) next

    tryCatch({
      fit <- coxph(
        Surv(time_dfs, event_dfs) ~ ic_env_6grp + age + female + chronic_n,
        data = dat
      )
      reri_rows[[coh]] <- compute_m5_reri(
        fit = fit,
        cohort = coh,
        n = nrow(dat),
        n_events = sum(dat$event_dfs)
      )
    }, error = function(e) {
      cat("    WARNING:", coh, "RERI failed:", conditionMessage(e), "\n")
    })
  }

  reri_stage1 <- bind_rows(reri_rows)
  if (nrow(reri_stage1) < 2) {
    stop("Fewer than two valid cohort-specific RERI estimates")
  }

  re_fit <- rma(yi = reri, sei = se, data = reri_stage1, method = "REML")
  pooled_reri <- tibble(
    model_source = "M5_6group",
    estimand = "RERI for low IC and low environment vs high IC and high environment",
    method = "cohort-specific delta method with REML pooling",
    reference_group = "High IC / High Env",
    k = re_fit$k,
    pooled_reri = as.numeric(re_fit$beta),
    se = as.numeric(re_fit$se),
    ci_lower = as.numeric(re_fit$ci.lb),
    ci_upper = as.numeric(re_fit$ci.ub),
    p_value = as.numeric(re_fit$pval),
    tau2 = re_fit$tau2,
    I2 = re_fit$I2,
    Q = re_fit$QE,
  ...
  write_csv(reri_stage1, file.path(RESULTS_DIR, "reri_stage1.csv"))
  write_csv(pooled_reri, file.path(RESULTS_DIR, "reri_stage2_pooled.csv"))

  print(reri_stage1 %>% select(cohort, reri, ci_lower, ci_upper, p_value))
  cat("\n    Pooled RERI =", sprintf("%.3f", pooled_reri$pooled_reri),
      "(", sprintf("%.3f", pooled_reri$ci_lower), "to",
      sprintf("%.3f", pooled_reri$ci_upper), ")",
      "p =", format(pooled_reri$p_value, digits = 3),
      "I² =", sprintf("%.1f%%", pooled_reri$I2), "\n")
  cat("    Interpretation:", ifelse(
    pooled_reri$pooled_reri > 0,
    "positive departure from additivity on the hazard-ratio scale",
    "sub-additive joint excess hazard on the hazard-ratio scale"
  ), "\n")
  cat("    Saved: results/reri_stage1.csv, results/reri_stage2_pooled.csv\n\n")
...
step7_summary <- function(surv, pooled_dfs, pooled_mort, pooled_sub) {
  ...
  cat("    results/reri_stage1.csv\n")
  cat("    results/reri_stage2_pooled.csv\n")
